...
export interface ChatSession { id: string; title: string; modelId: string; messages: UIMessage[]; createdAt: string; updatedAt: string; }
/** 会话列表使用的轻量摘要，不包含消息正文。 */
...
export function toSummary(session: ChatSession): SessionSummary {
  return { id: session.id, title: session.title, modelId: session.modelId, createdAt: session.createdAt, updatedAt: session.updatedAt };
}
/** 创建带默认模型和时间戳的新会话。 */
export function createSession(id = crypto.randomUUID()): ChatSession {
  const now = new Date().toISOString();
  return { id, title: "新对话", modelId: "deepseek-chat", messages: [], createdAt: now, updatedAt: now };
}
/** 使用首条用户消息生成简短的会话标题。 */
