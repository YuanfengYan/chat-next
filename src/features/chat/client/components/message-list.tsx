...
import { MessageItem } from "@/features/chat/client/components/message-item";

export function MessageList({ messages, isStreaming, onEdit }: { messages: UIMessage[]; isStreaming: boolean; onEdit: (messageId: string, text: string) => void }) {
        ...
        <div className="mx-auto w-full pb-6">
            {messages.map((message) => (
                <MessageItem
                    key={message.id}
                    message={message}
                    isEditing={!isStreaming && editingMessageId === message.id}
