...
import { resolveModel } from "@/features/ai/server/providers/provider-registry.server";
import { SKILL_CATALOG } from "@/features/ai/domain/skills";
import { SkillLoader } from "@/features/ai/server/skills/skill-loader.server";
import { createSkillTools, type SkillLogger } from "@/features/ai/server/skills/skill-tools.server";
import { createChatTools } from "@/features/ai/server/tools/chat-tools.server";
    ...
    async streamChat(input: StreamChatInput) {
        ...
        const dynamic =
            input.generationId && input.userId
                ? await import("@/features/ai/server/tools/dynamic-tool-registry.server").then(({ loadDynamicTools }) =>
